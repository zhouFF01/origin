...
bool FIRST_GPS=true;
/* // 全局变量定义
const int step_duration = 100;  // 每段噪声持续100帧（10秒）
const int step_interval = 1000; // 每隔1000帧（100秒）加入一次噪声

// 全局计数器和噪声变量
uint64_t step_counter = 0;
double step_noise_x = 0.0;
double step_noise_y = 0.0;
double step_noise_z = 0.0; */

/* // 将随机数生成器和分布移到全局范围
std::random_device rd;
std::mt19937 generator(rd());
std::normal_distribution<double> distribution(0.0, 10.0);  // 标准差为4米 */
VioManager::VioManager(VioManagerOptions &params_) : thread_init_running(false), thread_init_success(false) {



  file_state.open("/home/zhou/workspace/data/state.txt");
  file_gps.open("/home/zhou/workspace/data/gps.txt");
...
void VioManager::feed_measurement_imu(const ov_core::ImuData &message) {
 ...
 // std::cout << std::fixed << std::setprecision(15)<<"oldest_time "<< oldest_time << std::endl;
  //std::cout<< std::fixed << std::setprecision(15)<< "state_time " << state->_timestamp << std::endl;
...
//double timestamp_inC = message.timestamp - state->_calib_dt_CAMtoIMU->value()(0);state->_timestamp


// 循环处理 GPS 队列
double timestamp=message.timestamp;
while (!gps_queue.empty() && gps_queue.at(0).timestamp < timestamp) {
   

    if (is_initialized_vio && !did_zupt_update) {
        std::cout << "track gps and update" << std::endl;
   
        if ( FIRST_GPS)
        { latest_gps_data = gps_queue.at(0);
          track_gps_and_update(latest_gps_data);
          FIRST_GPS = false;//提取出第一个GPS数据作为初始值，来计算相对位置
...
void VioManager::track_image_and_update(const ov_core::CameraData &message_const) {
 std::lock_guard<std::mutex> lock(statecov_mutex);//锁定state？
  // 开始计时
  rT1 = boost::posix_time::microsec_clock::local_time();
...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
  ...
  //dx_global =updaterMSCKF->update(state, featsup_MSCKF);
  propagator->invalidate_cache();
...
Eigen::Vector3d VioManager::ConvertLonLatHeiToENU(const Eigen::Vector3d &init_lla, const Eigen::Vector3d &point_lla) {
        const double a = 6378137.0;  // WGS84 semi-major axis
        const double f = 1.0 / 298.257223563;  // WGS84 flattening
    
        const double e2 = 2.0 * f - f * f;  // eccentricity squared

         double lon0 = init_lla(0) * M_PI / 180.0;
        double lat0 = init_lla(1) * M_PI / 180.0;
        double h0 = init_lla(2);

        double lon = point_lla(0) * M_PI / 180.0;
        double lat = point_lla(1) * M_PI / 180.0;
        double h = point_lla(2);
        ...
        ecef0(0) = (N0 + h0) * cos(lat0) * cos(lon0);
        ecef0(1) = (N0 + h0) * cos(lat0) * sin(lon0);
        ecef0(2) = (N0 * (1.0 - e2) + h0) * sin(lat0);

        ecef(0) = (N + h) * cos(lat) * cos(lon);
        ecef(1) = (N + h) * cos(lat) * sin(lon);
        ecef(2) = (N * (1.0 - e2) + h) * sin(lat);
...
/*     // 使用全局变量 step_counter 来判断噪声状态
    if (step_counter % step_interval == 0) {
        // 生成新的台阶噪声
        step_noise_x = distribution(generator);
        step_noise_y = distribution(generator);
        step_noise_z = distribution(generator);
    }
 */
/*     // 只在 step_duration 次数内应用噪声
    if (step_counter % step_interval < step_duration) {
        enu(0) += step_noise_x;
        enu(1) += step_noise_y;
        enu(2) += step_noise_z;
    }  */
         // GPS to IMU transformation parameters
          double theta = 1.3174936595568707;
          double tx = -3.10601236;
          double ty = 36.36746244;

          // Create rotation matrix for GPS to IMU transformation
          Eigen::Matrix3d R_enu_to_imu;
          R_enu_to_imu << cos(theta), -sin(theta), 0,
                          sin(theta), cos(theta), 0,
                          0, 0, 1;

          // Create translation vector
          Eigen::Vector3d T_enu_to_imu(tx, ty, 0);

          // Apply translation and rotation to convert from ENU to IMU
         // Eigen::Vector3d imu = R_enu_to_imu * (enu - T_enu_to_imu);
Eigen::Vector3d imu = R_enu_to_imu * enu;
 // 更新计数器
    //step_counter++;
          return imu;
...
{  
  ...
  Eigen::Vector3d point1(126.9408762, 37.5204112, 45.764);

  Eigen::Vector3d G_p_Gps = ConvertLonLatHeiToENU(latest_gps_data.lla, message.lla);
//std::cout <<"imu_pos"  << state->_imu->pos() << std::endl;
   
std::cout << std::fixed << std::setprecision(15)<<"gps_time:"  << message.timestamp  <<std::endl;
std::cout << std::fixed << std::setprecision(15)<<"state_time:"  << state->_timestamp  <<std::endl;

std::cout << std::fixed << std::setprecision(15)<<"TimeOffset:"  << message.timestamp-state->_timestamp  <<std::endl;
/*  if (state->_timestamp != message.timestamp) {
    propagator->propagate_and_clone_gps(state, message.timestamp-0.01);//递推状态时间戳到GPS时间戳，减去0.01s是为了避免一开始时间戳判断进入死循环
 }  */
  file_gps << std::fixed << std::setprecision(6) <<state->_timestamp<< " " <<G_p_Gps[0] << " " << G_p_Gps[1] << " " << G_p_Gps[2] << std::endl;
...
//{
 ...
 //StateHelper::GpsUpdate(state, Hx, res, cov);

  Eigen::MatrixXd P_minus_updated_huber, P_minus_updated_chi_square;
  Eigen::VectorXd x_huber, x_chi_square;
    // 运行两个滤波器
  // StateHelper::HuberFilter(state, Hx, res, cov, P_minus_updated_huber, x_huber);
     // 重置标志
       // EKF_updated = false;
//}
//StateHelper::ChiSquareFilter(state, Hx, res, cov, P_minus_updated_huber, x_huber);
StateHelper::ImmFilter(state, Hx, res, cov);
  return true;
}
