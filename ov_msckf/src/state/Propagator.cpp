...
void Propagator::propagate_and_clone_gps(std::shared_ptr<State> state, double timestamp) {
    // 如果当前更新时间与状态时间戳相同，报错并退出
    if (state->_timestamp == timestamp) {
        PRINT_ERROR(RED "Propagator::propagate_and_clone(): Propagation called again at same timestep at last update timestep!!!!\n" RESET);
        std::exit(EXIT_FAILURE);
    }
    ...
    // 执行随机克隆（stochastic cloning）
    StateHelper::augment_clone(state, last_w);

    std::cout << "完成IMU数据递推到时间戳: " << timestamp << std::endl;
}
