...
public:
  ...
  static void GpsUpdate(std::shared_ptr<State> state, const Eigen::MatrixXd &H, const Eigen::VectorXd &res, const Eigen::MatrixXd &R,Eigen::MatrixXd &P_updated, Eigen::VectorXd &x_updated);
  static void ImmFilter(std::shared_ptr<State> state, const Eigen::MatrixXd &H, const Eigen::VectorXd &res, const Eigen::MatrixXd &R);
  static void HuberFilter(std::shared_ptr<State> state, const Eigen::MatrixXd &H, const Eigen::VectorXd &z, const Eigen::MatrixXd &R,Eigen::MatrixXd &P_updated, Eigen::VectorXd &x_updated);
  static void ChiSquareFilter(std::shared_ptr<State> state, const Eigen::MatrixXd &H, const Eigen::VectorXd &res, const Eigen::MatrixXd &R,Eigen::MatrixXd &P_updated, Eigen::VectorXd &x_updated);
    // 计算给定自由度和置信水平的卡方阈值
    static double ChiSquareThreshold(int degrees_of_freedom, double confidence_level);
